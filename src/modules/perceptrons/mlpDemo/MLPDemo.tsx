...
const MLPDemo = (props: { labelColor: string }) => {
  ...
  const changeNeuronValue = useCallback(
    (layer: number, neuron: number, key: string, value: any) => {
  ...
  const getNeuronOutputs = useCallback(
    (inputs) => {
    ...
    (clickedX, clickedY) => {
      setInputs((inp) => {
        ...
        return newInputs.length === inp.length
          ? newInputs.concat([
              [clickedX, clickedY, calculatePointColor(clickedX, clickedY)],
            ])
          : newInputs;
      });
    },

...
const RblattVectorsDemo = () => {
    ...
    useEffect(() => {
        ...
        setPerpPoint(pA);
        const perpArrow = newBoard.create('arrow', [demoConfig.MAINLINE_INTERSECTION, pA],
            { strokeColor: 'black', strokeWidth: 2, fixed: true, name: 'Wt', withLabel:true, label: {position: 'top'} });
        const li = newBoard.create('perpendicular', [perpArrow, demoConfig.MAINLINE_INTERSECTION],
    ...
    const goNext = () => {
        ...
        switch (phase) {
            case 0:
                ...
                const dist = board?.create('arrow', [demoConfig.MAINLINE_INTERSECTION, demoConfig.CURR_POINT],
            ...
            case 1:
                board?.create('arrow', [demoConfig.INIT_PERP_POINT, demoConfig.NEW_PERP_POINT],
                     { strokeColor: 'black', strokeWidth: 2, fixed: true, dash: 2, name: '-d', withLabel:true, label: {position: 'top'}});
                board?.create('arrow', [demoConfig.MAINLINE_INTERSECTION, demoConfig.NEW_PERP_POINT],
        ...
        switch (phase) {
            ...
            case 3:
                ...
                perpPoint.moveTo(demoConfig.INIT_PERP_POINT, 700, {callback: () => {
                    ...
                    board?.create('arrow', [demoConfig.MAINLINE_INTERSECTION, demoConfig.CURR_POINT],
                    { strokeColor: 'black', strokeWidth: 2, fixed: true, dash: 2, name: 'd', withLabel:true, label: {position: 'top'}});
                    board?.create('arrow', [demoConfig.INIT_PERP_POINT, demoConfig.NEW_PERP_POINT],
                     { strokeColor: 'black', strokeWidth: 2, fixed: true, dash: 2, name: '-d', withLabel:true, label: {position: 'top'}});
                }});
...
const translationConfig = {
    POINTS: [
        {x: 3.481875, y: 4.48, z: 0},
        {x: 5.161875, y: 4.52, z: 0},
        {x: 4.801875, y: 3.44, z: 1},
        {x: 3.161875, y: 0.88, z: 1},
        {x: 3.721875, y: 1.84, z: 1},
        {x: 5.281875, y: .96, z: 1}],
    INIT_PERP_POINT: [5.22, 4.04],
    NEW_PERP_POINT: [5.22, 4.04],
    MAINLINE_INTERSECTION: [4.5, 2.5],
    CURR_POINT: [4.801875, 3.44]
};

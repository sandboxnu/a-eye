...
const filterDescriptions: string[] = [
  "-760 is negative which indicates there is no diagonal down, dark to light edge at this pixel.",
  "760 is positive and large which indicates a prominent diagonal down, light to dark edge at this pixel.",
...
const ZoomedSobelExample: React.FC<ZoomedSobelType> = ({ labelColor }) => {
  const [filterIdx, setFilterIdx] = useState<number>(1);
  const [step, setStep] = useState<number>(0);

  return (
    <div className={`${labelColor} container flex flex-col border-2 border-navy border-opacity-50 justify-center mx-auto`}>
      ...
      <div className="my-5 mx-2">
        ...
        }
        {
          ...
          <div>
            ...
            <div>
              ...
              <select className=" bg-white bg-opacity-50" id="filters" name="filters" defaultValue={1} onChange={(event) => { setFilterIdx(parseInt(event.target.value)) }}>
                {
        ...
        }
        {
          step === 2 &&
          <div>
            <p>For each direction, we can draw a needle with the length of the sobel filter's scalar magnitude, rotated to the appropriate angle.</p>
            <img src={sobelToNeedleChart} alt="sobelNeedles" className="w-3/4 mx-auto " />
            <p>We combine all the needles together to get a quick sense of the edges present at that pixel.</p>
          </div>
        }
      </div>
    </div>
